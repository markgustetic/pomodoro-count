...
- **Tested logic is extracted from SwiftUI.** `Reorder.destination`,
  ...
  `CategoryAdvance.next(after:in:)`, `StatusIcon.glyph(phase:running:)`,
  `HistoryReadout.tooltip` and `TooltipPlacement.origin` are pure and
  unit-tested; views are thin over them. Follow that shape: new behavior
  gets a failing test first, in `Tests/PomodoroCountTests` (swift-testing,
  not XCTest). The glyph is the clearest case for why: it was lifted out of
  the drawing routine so that a phase arriving without its own glyph rule
  fails a test, instead of waiting for someone to notice the wrong icon in
  the menu bar.
